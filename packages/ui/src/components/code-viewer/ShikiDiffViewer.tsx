...
import { registerOPCAgentShikiThemes } from './registerShikiThemes'
...
export interface ShikiDiffViewerProps {
   ...
   *  Shiki theme natively. Falls back to opcagent-dark/opcagent-light (transparent bg) if not set. */
  shikiTheme?: string
...
export function ShikiDiffViewer({
  ...
  // to opcagent-dark/opcagent-light which have transparent bg for CSS variable theming
  const resolvedThemeName = shikiTheme || (theme === 'dark' ? 'opcagent-dark' : 'opcagent-light')
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      ;(header as any).__opcagentClickCleanup = () => header.removeEventListener('click', handleClick)
    }, 150)
    ...
    return () => {
      ...
      const header = diffsContainer?.shadowRoot?.querySelector('[data-diffs-header]')
      if (header) {
        ;(header as any).__opcagentClickCleanup?.()
      }
    }
