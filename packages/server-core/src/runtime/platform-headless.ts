...
function createConsoleLogger(): Logger {
  ...
  return {
    ...
    debug: (...args) => {
      if (process.env.OPCAGENT_DEBUG === 'true' || process.env.OPCAGENT_IS_PACKAGED !== 'true') {
 ...
 * - OPCAGENT_APP_ROOT — override appRootPath (default: cwd)
 * - OPCAGENT_RESOURCES_PATH — override resourcesPath (default: cwd/resources)
 * - OPCAGENT_IS_PACKAGED — 'true' for production (default: false)
 * - OPCAGENT_VERSION — app version string (default: '0.0.0-dev')
 * - OPCAGENT_DEBUG — 'true' to enable debug logging
 */
export function createHeadlessPlatform(options?: { appVersion?: string }): PlatformServices {
  const logger = createConsoleLogger()
  const isDebugMode = process.env.OPCAGENT_DEBUG === 'true' || process.env.OPCAGENT_IS_PACKAGED !== 'true'

  return {
    appRootPath: process.env.OPCAGENT_APP_ROOT || process.cwd(),
    resourcesPath: process.env.OPCAGENT_RESOURCES_PATH || join(process.cwd(), 'resources'),
    isPackaged: process.env.OPCAGENT_IS_PACKAGED === 'true',
    appVersion: process.env.OPCAGENT_VERSION || options?.appVersion || '0.0.0-dev',
