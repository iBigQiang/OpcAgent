...
export const DEBUG = INTERCEPTOR_LOGGING_ENABLED &&
  (process.argv.includes('--debug') || process.env.OPCAGENT_DEBUG === '1');
...
let _sessionDir: string | null = process.env.OPCAGENT_SESSION_DIR || null;
 ...
 * - SDK subprocess: OPCAGENT_SESSION_DIR env var (set by main process before spawn)
