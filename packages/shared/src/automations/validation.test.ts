...
describe('validateAutomationsConfig', () => {
    it('rejects webhook actions on agent events instead of silently ignoring them', () => {
      const result = validateAutomationsConfig({ automations: { PreToolUse: [{ actions: [{ type: 'webhook', url: 'https://1.1.1.1/hook' }] }] } });
      expect(result.valid).toBe(false);
      expect(result.errors.some(error => error.includes('prompt actions only'))).toBe(true);
    });
