...
import { expandEnvVars, matcherMatchesSdk, parsePromptReferences } from './utils.ts';
...
export class AutomationSystem implements AutomationsConfigProvider {
  ...
  /** Serialize updates for each session so concurrent identical snapshots cannot emit duplicate events. */
  private readonly metadataUpdates: Map<string, Promise<void>> = new Map();
  ...
  async updateSessionMetadata(
    ...
    const previous = this.metadataUpdates.get(sessionId) ?? Promise.resolve();
    let resolveEvents!: (events: AppEvent[]) => void;
    let rejectEvents!: (error: unknown) => void;
    const events = new Promise<AppEvent[]>((resolve, reject) => {
      resolveEvents = resolve;
      rejectEvents = reject;
    });
    const update = previous.then(async () => {
      try {
        resolveEvents(await this.updateSessionMetadataInternal(sessionId, next));
      } catch (error) {
        rejectEvents(error);
      }
    });
    const settledUpdate = update.catch(() => undefined);
    this.metadataUpdates.set(sessionId, settledUpdate);
    void update.finally(() => {
      if (this.metadataUpdates.get(sessionId) === settledUpdate) this.metadataUpdates.delete(sessionId);
    });
    return events;
  }

  private async updateSessionMetadataInternal(
    sessionId: string,
    next: SessionMetadataSnapshot,
  ): Promise<AppEvent[]> {
    if (this.disposed) return [];
  ...
  async executeAgentEvent(event: AgentEvent, input: SdkAutomationInput, signal?: AbortSignal): Promise<number> {
    if (this.disposed || signal?.aborted || !this.config) return 0;
    ...
    for (const matcher of matchers) {
      ...
      for (const action of matcher.actions) {
        ...
        pendingPrompts.push({
          ...
          sourceEvent: event,
          prompt,
          mentions: parsePromptReferences(prompt).mentions,
          labels: matcher.labels?.map(label => expandEnvVars(label, env)),
          permissionMode: matcher.permissionMode,
          llmConnection: action.llmConnection,
          model: action.model,
          thinkingLevel: action.thinkingLevel,
          telegramTopic: matcher.telegramTopic ? expandEnvVars(matcher.telegramTopic, env).trim() || undefined : undefined,
        });
      }
      if (pendingPrompts.length > 0 && !signal?.aborted) this.options.onPromptsReady?.(pendingPrompts);
    }
   ...
   *
   * Each callback delegates to the same prompt-only agent-event execution path.
   */
  buildSdkHooks(): Partial<Record<AgentEvent, SdkAutomationCallbackMatcher[]>> {
    ...
    for (const event of AGENT_EVENTS) {
      ...
      hooks[event] = matchers.map(matcher => ({
        ...
        hooks: [async (input, _toolUseId, options) => {
          await this.executeAgentEvent(event, input, options.signal);
