...
import { isValidCron } from './cron-matcher.ts';
import type { ValidationResult, ValidationIssue } from '../config/validators.ts';
import { AGENT_EVENTS, type AgentEvent, type AutomationsConfig, type AutomationsValidationResult } from './types.ts';
...
function runMatcherSemanticValidations(
  ...
  for (const [event, matchers] of Object.entries(config.automations)) {
    ...
    for (let i = 0; i < matchers.length; i++) {
      ...
      if (!matcher) continue;
      if (AGENT_EVENTS.includes(event as AgentEvent) && matcher.actions.some(action => action.type === 'webhook')) {
        errors.push({
          file,
          path: `automations.${event}[${i}].actions`,
          message: 'Agent events support prompt actions only',
          severity: 'error',
          suggestion: 'Move the webhook to an app event or replace it with a prompt action',
        });
      }
      ...
      if (matcher.cron) {
        try {
          if (!isValidCron(matcher.cron)) throw new Error('Expected a valid five-field cron expression');
        } catch (e) {
          errors.push({
            file,
        ...
        for (let j = 0; j < matcher.actions.length; j++) {
          ...
          if (action && typeof action === 'object' && 'type' in action && action.type === 'webhook' && 'url' in action && typeof action.url === 'string' && action.url.includes('$')) {
            warnings.push({
              ...
              severity: 'warning',
              suggestion: 'Ensure the referenced OPCAGENT_WH_* variables are set in your shell profile',
            });
          }
        }
      }
