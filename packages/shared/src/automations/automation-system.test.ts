...
describe('AutomationSystem', () => {
  ...
  describe('updateSessionMetadata', () => {
    ...
    it('serializes concurrent identical updates to keep event delivery idempotent', async () => {
      const system = new AutomationSystem({
        workspaceRootPath: tempDir,
        workspaceId: 'test-workspace',
      });
      ...
      const [first, second] = await Promise.all([
        system.updateSessionMetadata('session-1', { labels: ['label-1'] }),
        system.updateSessionMetadata('session-1', { labels: ['label-1'] }),
      ]);

      expect(first).toEqual(['LabelAdd']);
  ...
  describe('executeAgentEvent', () => {
    it('should execute prompt actions when matcher and conditions pass', async () => {
      writeFileSync(join(tempDir, AUTOMATIONS_CONFIG_FILE), JSON.stringify({
        automations: {
          PreToolUse: [
            {
      ...
      const prompts: import('./types.ts').PendingPrompt[] = [];
      const system = new AutomationSystem({
        workspaceRootPath: tempDir,
        workspaceId: 'test-workspace',
        onPromptsReady: items => prompts.push(...items),
      });
      ...
      expect(prompts[0]?.prompt).toBe('check this');
      await system.dispose();
    });
  ...
  describe('buildSdkHooks', () => {
    it('should return prompt-only callbacks for configured agent events', async () => {
      writeFileSync(join(tempDir, AUTOMATIONS_CONFIG_FILE), JSON.stringify({
        automations: {
          PreToolUse: [
      ...
      await result.PreToolUse?.[0]?.hooks[0]?.({ hook_event_name: 'PreToolUse', tool_name: 'Bash' }, 'tool-1', {});
      expect(prompts).toHaveLength(1);
  ...
  describe('dispose', () => {
    it('does not restart the scheduler after disposal', async () => {
      const system = new AutomationSystem({
        workspaceRootPath: tempDir,
        workspaceId: 'test-workspace',
    ...
    it('does not emit or retain metadata after cleanup', async () => {
      const system = new AutomationSystem({ workspaceRootPath: tempDir, workspaceId: 'test-workspace' });
      const handler = mock(() => undefined);
      system.eventBus.on('LabelConfigChange', handler);
      await system.dispose();
      await system.emitLabelConfigChange();
      await system.updateSessionMetadata('session-after-cleanup', { labels: ['label'] });
      expect(handler).not.toHaveBeenCalled();
      expect(system.getSessionMetadata('session-after-cleanup')).toBeUndefined();
    });
  });
});
