...
export interface RetrySchedulerOptions {
  ...
  /** Injectable for deterministic tests; production uses the shared webhook executor. */
  webhookExecutor?: typeof executeWebhookRequest;
}

export class RetryScheduler {
  private readonly workspaceRootPath: string;
  private readonly webhookExecutor: typeof executeWebhookRequest;
  private timer: ReturnType<typeof setInterval> | null = null;
  private initialTickTimer: ReturnType<typeof setTimeout> | null = null;
  private processing = false;
  private disposed = false;
  /** Serialize queue-file mutations so enqueue cannot be overwritten by a concurrent tick. */
  ...
  constructor(options: RetrySchedulerOptions) {
    ...
    this.webhookExecutor = options.webhookExecutor ?? executeWebhookRequest;
  }
  ...
  private async tick(): Promise<void> {
    ...
    try {
      await this.withQueueLock(() => this.processQueue());
    } catch (err) {
  ...
  /** Test-only deterministic queue processing entry point. */
  async tickForTest(): Promise<void> {
    await this.tick();
  }

  private async withQueueLock(operation: () => Promise<void>): Promise<void> {
    const next = this.queueOperation.then(operation, operation);
    this.queueOperation = next.catch(() => undefined);
    await next;
  }
  ...
  private async processQueue(): Promise<void> {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        try {
          result = await this.webhookExecutor(entry.action, { timeoutMs: 30_000 });
        } catch (err) {
          result = {
            type: 'webhook',
      ...
      log.debug(`[RetryScheduler] Queue processing error: ${err}`);
    }
  }
