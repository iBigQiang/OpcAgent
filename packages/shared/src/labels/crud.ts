...
import { initializeLabelConfig, loadLabelConfig, saveLabelConfig, isValidLabelId, isValidLabelIdFormat } from './storage.ts';
...
export function createLabel(
  ...
  if (!input.name.trim()) throw new Error('Label name is required');
  const config = initializeLabelConfig(workspaceRootPath);
...
export function updateLabel(
  ...
  if (updates.autoRules !== undefined) label.autoRules = updates.autoRules;
...
export async function deleteLabel(
  ...
  for (const id of removedIds) {
    stripped += await stripLabelFromSessions(workspaceRootPath, id);
  }
...
export function reorderLabels(
  ...
  if (orderedIds.length !== siblings.length || requestedIds.size !== siblings.length) {
    throw new Error('Reorder must include each sibling exactly once');
  }
  for (const id of requestedIds) {
    if (!siblingIds.has(id)) throw new Error(`Invalid label ID for reorder: '${id}'`);
  }
...
export function moveLabel(
  ...
  if (newParentId === labelId) {
    throw new Error(`Cannot move label '${labelId}' into itself`);
  }
 ...
 */
async function stripLabelFromSessions(
  workspaceRootPath: string,
  ...
  for (const session of sessions) {
    ...
    if (session.labels && session.labels.some((entry: string) => extractLabelId(entry) === deletedLabelId)) {
      ...
      await updateSessionMetadata(workspaceRootPath, session.id, { labels: updatedLabels });
      strippedCount++;
    }
  }
