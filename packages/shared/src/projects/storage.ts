...
const PROJECT_ID_PATTERN = /^proj_[a-f0-9]{8}$/;

/** True when a slug is a generated project directory name, not a path. */
export function isValidProjectSlug(projectSlug: string): boolean {
  return projectSlug.length <= 50 && PROJECT_SLUG_PATTERN.test(projectSlug);
}

/** True when an id has the format generated by createProject(). */
export function isValidProjectId(projectId: string): boolean {
  return PROJECT_ID_PATTERN.test(projectId);
}

function assertProjectSlug(projectSlug: string): void {
  if (!isValidProjectSlug(projectSlug)) throw new Error(`Invalid project slug: ${projectSlug}`);
}
...
function isValidProjectConfig(config: unknown, projectSlug: string): config is ProjectConfig {
  if (!config || typeof config !== 'object' || Array.isArray(config)) return false;
  const value = config as Record<string, unknown>;
  if (typeof value.id !== 'string' || !isValidProjectId(value.id)) return false;
  if (typeof value.slug !== 'string' || value.slug !== projectSlug || !isValidProjectSlug(value.slug)) return false;
  ...
  for (const key of ['description', 'workingDirectory', 'details', 'colorTheme', 'color'] as const) {
    if (value[key] !== undefined && typeof value[key] !== 'string') return false;
  }
  return value.archivedAt === undefined || Number.isFinite(value.archivedAt);
}

function assertProjectConfig(config: ProjectConfig): void {
  ...
  if (typeof slug !== 'string') throw new Error('Invalid project config');
  assertProjectSlug(slug);
  if (!isValidProjectConfig(raw, slug)) throw new Error(`Invalid project config: ${slug}`);
}
...
export function loadProjectConfig(
  ...
  try {
    ...
    if (!isValidProjectConfig(config, projectSlug)) {
      debug('[loadProjectConfig] Invalid project config:', projectSlug);
      return null;
    }

    // Expand portable paths on read so consumers always see absolute paths.
    const loadedConfig = { ...config };
    if (loadedConfig.workingDirectory) {
      loadedConfig.workingDirectory = expandPath(loadedConfig.workingDirectory);
    }
...
/**
 * Resolve the exact project context authorized for model delivery.
 * Reads MEMORY.md and asset file names only; never reads asset contents and
 * never creates a missing assets directory.
 */
export function loadProjectPromptContext(
  ...
  try {
    ...
    const assetsPath = getProjectAssetsPath(workspaceRootPath, project.config.slug);
    const assetFilenames = existsSync(assetsPath)
      ...
      : [];
    return {
      memoryContent: loadProjectMemory(workspaceRootPath, project.config.slug) ?? undefined,
      assetFilenames,
    };
  } catch (error) {
    debug('[loadProjectPromptContext] Failed to resolve project context:', error);
    return null;
  }
}
