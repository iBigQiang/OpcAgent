...
import { isPlatformSenderAllowed, isSenderAllowed, seedFirstPlatformOwner, setPlatformOwners } from './access-control'
...
import { TelegramAdapter, TelegramConnectionError } from './adapters/telegram'
...
interface WorkspaceState {
  ...
  pendingUnbound: Map<string, PendingUnboundMessage>
  adapters: Partial<Record<PlatformType, PlatformAdapter>>
}

interface PendingUnboundMessage {
  message: import('./types').IncomingMessage
  expiresAt: number
  optionTokens: Map<string, string>
  promptMessageIds: string[]
}

const UNBOUND_MESSAGE_TTL_MS = 10 * 60_000
const MAX_PENDING_UNBOUND_MESSAGES = 50
...
export class MessagingGatewayRegistry {
  ...
  generateOwnerPairingCode(workspaceId: string, platform: 'telegram'): { code: string; expiresAt: number; botUsername?: string } { if (platform !== 'telegram') throw new Error('Workspace owner pairing is only available for Telegram'); const entry = this.pairing.createOwner(workspaceId, platform); return { code: entry.code, expiresAt: entry.expiresAt, botUsername: this.getConfig(workspaceId).platforms.telegram?.botUsername } }
  ...
  async saveTelegramToken(workspaceId: string, token: string): Promise<void> { await this.saveCredential(workspaceId, 'telegram', token); this.enablePlatform(workspaceId, 'telegram'); await this.connect(workspaceId, 'telegram'); const runtime = this.getRuntime(workspaceId).find(value => value.platform === 'telegram'); if (!runtime?.connected) throw new Error(runtime?.lastError ?? 'Telegram token was saved, but the bot could not connect.') }
  ...
  private pendingUnboundScope(message: Pick<import('./types').IncomingMessage, 'platform' | 'channelId' | 'threadId' | 'senderId'>): string { return `${message.platform}:${message.channelId}:${message.threadId ?? ''}:${message.senderId}` }
  private opaqueToken(): string { return randomUUID().replace(/-/g, '').slice(0, 16) }
  private async clearUnboundButtons(adapter: PlatformAdapter, entry: PendingUnboundMessage): Promise<void> { for (const messageId of entry.promptMessageIds) await adapter.clearButtons?.(entry.message.channelId, messageId).catch(() => undefined) }
  private async promptUnboundMessage(workspaceId: string, message: import('./types').IncomingMessage, adapter: PlatformAdapter): Promise<void> {
    if (message.platform !== 'telegram' || message.chatType !== 'private' || !adapter.sendButtons) return
    const state = this.state(workspaceId)
    const scope = this.pendingUnboundScope(message)
    const now = this.now()
    for (const [token, entry] of state.pendingUnbound) if (entry.expiresAt <= now || this.pendingUnboundScope(entry.message) === scope) { state.pendingUnbound.delete(token); await this.clearUnboundButtons(adapter, entry) }
    while (state.pendingUnbound.size >= MAX_PENDING_UNBOUND_MESSAGES) { const oldest = state.pendingUnbound.entries().next().value; if (!oldest) break; state.pendingUnbound.delete(oldest[0]); await this.clearUnboundButtons(adapter, oldest[1]) }
    const token = this.opaqueToken()
    const entry: PendingUnboundMessage = { message, expiresAt: now + UNBOUND_MESSAGE_TTL_MS, optionTokens: new Map(), promptMessageIds: [] }
    state.pendingUnbound.set(token, entry)
    const prompt = await adapter.sendButtons(message.channelId, '这条消息还没有绑定会话。请选择：使用默认模型新建会话 / 选择已有会话。', [
      { id: 'unbound:new', label: '使用默认模型新建会话', data: `unbound:new:${token}` },
      { id: 'unbound:choose', label: '选择已有会话', data: `unbound:choose:${token}` },
    ], message.threadId === undefined ? undefined : { threadId: message.threadId })
    if (prompt && typeof prompt === 'object' && 'messageId' in prompt && typeof prompt.messageId === 'string') entry.promptMessageIds.push(prompt.messageId)
  }
  private async handleUnboundButton(workspaceId: string, press: ButtonPress, action: string | undefined): Promise<void> {
    if (press.platform !== 'telegram' || !action) return
    const state = this.state(workspaceId)
    const [, , token, optionToken] = press.buttonId.split(':')
    if (!token) return
    const pending = state.pendingUnbound.get(token)
    const adapter = state.adapters.telegram
    if (!pending) return
    if (pending.expiresAt <= this.now()) { state.pendingUnbound.delete(token); if (adapter) await this.clearUnboundButtons(adapter, pending); return }
    if (this.pendingUnboundScope(pending.message) !== this.pendingUnboundScope(press) || !adapter?.isConnected() || !isPlatformSenderAllowed(state.config.get(), 'telegram', press.senderId)) return
    if (action === 'choose') {
      const sessions = this.options.sessionManager?.getSessions(workspaceId).filter(session => !session.isArchived).sort((left, right) => right.lastMessageAt - left.lastMessageAt).slice(0, 8) ?? []
      if (!sessions.length || !adapter.sendButtons) { await adapter.sendText(press.channelId, 'No existing chats are available. Choose New chat instead.', press.threadId === undefined ? undefined : { threadId: press.threadId }); return }
      pending.optionTokens.clear()
      const rows = sessions.map(session => { const option = this.opaqueToken(); pending.optionTokens.set(option, session.id); return { id: `unbound:session:${option}`, label: (session.name || session.id).slice(0, 64), data: `unbound:session:${token}:${option}` } })
      const selection = await adapter.sendButtons(press.channelId, '选择已有会话。', rows, press.threadId === undefined ? undefined : { threadId: press.threadId })
      if (selection && typeof selection === 'object' && 'messageId' in selection && typeof selection.messageId === 'string') pending.promptMessageIds.push(selection.messageId)
      return
    }
    let sessionId: string | undefined
    if (action === 'new') sessionId = (await this.options.sessionManager?.createSession(workspaceId))?.id
    else if (action === 'session' && optionToken) {
      const selectedId = pending.optionTokens.get(optionToken)
      if (!selectedId) return
      const selected = this.options.sessionManager?.getSessions(workspaceId).find(session => session.id === selectedId && !session.isArchived)
      sessionId = selected?.id
    }
    if (!sessionId) return
    state.pendingUnbound.delete(token)
    await this.clearUnboundButtons(adapter, pending)
    state.bindings.bind({ workspaceId, sessionId, platform: 'telegram', channelId: pending.message.channelId, ...(pending.message.threadId === undefined ? {} : { threadId: pending.message.threadId }), ...(pending.message.channelName ? { channelName: pending.message.channelName } : {}) })
    this.publish('messaging:bindingChanged', workspaceId)
    await state.router.route(pending.message, adapter)
  }
}
