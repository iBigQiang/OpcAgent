...
import type { IncomingMessage, MessagingPlatformConfig, PlatformAdapter, PlatformRuntimeInfo } from '../../types'
...
export class WhatsAppAdapter implements PlatformAdapter {
  ...
  constructor(private readonly options: { workerEntry: string; authDir: string; nodeBin?: string; electronRunAsNode?: boolean }) {}
  ...
  async initialize(_input: { credential: string; config: MessagingPlatformConfig }): Promise<void> {
    if (this.proc) return
  ...
  async sendText(channelId: string, text: string): Promise<void> { this.send({ type: 'send_text', requestId: crypto.randomUUID(), channelId, text }) }
  private startWorker(): void {
    ...
    this.send({ type: 'start', authDir: this.options.authDir, pairingMode: 'qr' })
  }
  ...
  private consume(chunk: string): void { const parsed = parseFrames<WorkerEvent>(this.buffer + chunk); this.buffer = parsed.rest; for (const event of parsed.messages) { if (event.type === 'qr') this.statusHandler?.({ state: 'connecting', qrCode: event.value }); if (event.type === 'connected') { this.connected = true; this.statusHandler?.({ connected: true, state: 'connected', identity: event.jid, qrCode: undefined }) } if (event.type === 'disconnected') { this.connected = false; this.statusHandler?.({ connected: false, state: 'disconnected', lastError: event.reason, qrCode: undefined }) } if (event.type === 'unavailable') { this.connected = false; this.statusHandler?.({ connected: false, state: 'error', lastError: event.message, qrCode: undefined }) } if (event.type === 'incoming') void this.handler?.({ platform: 'whatsapp', channelId: event.channelId, messageId: event.messageId, senderId: event.senderId, senderName: event.senderName, text: event.text, timestamp: event.timestamp, raw: undefined }) } }
}
