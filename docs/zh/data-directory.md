...
默认根目录是 `~/.opcagent`(首次启动时由 `$HOME` 或 `%USERPROFILE%` 解析)。通过 `CONFIG_DIR` 可以隔离测试、运行多实例或备份到自定义位置。OPC Agent 不会读取或迁移任何其他产品的数据。
...
~/.opcagent/
  ...
  tool-icons/                       # 已知工具的图标
  logs/                             # 滚动 server 日志(`opcagent-server-*.log`)
...
请在应用停止**之后**或会话落盘之后再备份。开发期无需特殊 flush;生产用户应该先退出 OPC Agent,然后 `cp -R ~/.opcagent <backup>` 再重启。

## 平台路径

- macOS:`~/` 展开为 `/Users/<you>`,目录是隐藏的。
- Linux:`~/.opcagent`;若设置了 `$XDG_DATA_HOME`,renderer 仍走 `~/.opcagent` 保持向后兼容(此变化记录在 `migration/migration-features.md`)。
- Windows:`%USERPROFILE%\.opcagent`(即 `C:\Users\<you>\.opcagent`)。

## 配置损坏时的恢复

首次启动若 `~/.opcagent/config.json` 不可读,会把文件备份成 `config.json.broken-<timestamp>` 并回退到 `apps/electron/resources/config-defaults.json`。每个 workspace 的 `config.json` 同理:workspace 不会加载,而是用默认配置重建。
