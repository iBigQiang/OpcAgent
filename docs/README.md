# OPC Agent documentation (English)

OPC Agent is a cross-platform, local-first AI agent workspace derived from the MkAgent source lineage, which includes [Craft Agents OSS](https://github.com/craft-ai-agents/craft-agents-oss) `v0.11.2`. The current agent runtime is Pi-based: `@earendil-works/pi-coding-agent` runs in a dedicated Bun subprocess and serves the Electron app, WebUI, CLI, and headless server through one RPC contract. OPC Agent supports API-key and subscription connections, Ollama, AgentRouter, AnyRouter, custom OpenAI Chat / OpenAI Responses / Anthropic Messages / Google Gemini endpoints, API/MCP/local-folder Sources, projects, automations, and optional Telegram, WhatsApp, and Lark / Feishu messaging. It does not bundle the Claude Agent SDK, GitHub Copilot, Viewer/public sharing, or an image-generation agent tool.
