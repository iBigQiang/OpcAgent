...
export class WindowManager {
  ...
  private eventSink: ((channel: string, target: import('@opcagent/shared/protocol').PushTarget, ...args: any[]) => void) | null = null
   ...
   */
  setRpcEventSink(
    sink: (channel: string, target: import('@opcagent/shared/protocol').PushTarget, ...args: any[]) => void,
  ...
  getRpcEventSink(): ((channel: string, target: import('@opcagent/shared/protocol').PushTarget, ...args: any[]) => void) | null {
   ...
   *   1 window  → app name ("OPC Agent") on the lone window
  ...
  createWindow(options: CreateWindowOptions): BrowserWindow {
    ...
    // The renderer's index.html ships with `<title>OPC Agent</title>`, so
