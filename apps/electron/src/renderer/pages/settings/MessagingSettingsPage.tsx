...
function PlatformRow({ platform, workspaceId }: { platform: Platform; workspaceId: string }) {
  ...
  React.useEffect(() => {
    let cancelled = false
    window.electronAPI.getMessagingRuntime().then((items) => {
      if (cancelled) return
      const next = items.find((item) => item.platform === platform)
      setRuntime((next ?? defaultRuntime(platform)) as MessagingPlatformRuntimeInfo)
    })
    const off = window.electronAPI.onMessagingPlatformStatus((wsId, update) => {
      if (wsId !== workspaceId) return
      const status = (Array.isArray(update) ? update : [update]).find((item) => item.platform === platform)
                ...
                {platform === 'telegram' ? (
                  <>
                    <StyledDropdownMenuItem onClick={() => runAfterMenuClose(handleReconfigure)}>
                      ...
                      <span>{t('settings.messaging.telegram.configure')}</span>
                    </StyledDropdownMenuItem>
