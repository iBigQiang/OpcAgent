...
import { LabelIcon, LabelValueTypeIcon, resolveLabelColor } from './label-icon'
    ...
    <div className={cn('flex flex-wrap gap-1 px-4 pt-3 pb-1', className)}>
      {sessionLabels.map((entry, index) => {
        ...
        const resolvedColor = resolvedConfig.color
          ? resolveLabelColor(resolvedConfig.color, isDark)
          : 'var(--foreground)'
        ...
        return (
          <LabelValuePopover
            ...
            onOpenChange={(open) => { if (onLabelsChange) setOpenIndex(open ? index : null) }}
          ...
          >
            <MetadataBadge
              ...
              interactive={Boolean(onLabelsChange)}
              isActive={Boolean(onLabelsChange) && openIndex === index}
              showChevron={Boolean(onLabelsChange)}
            />
          </LabelValuePopover>
        )
      })}
