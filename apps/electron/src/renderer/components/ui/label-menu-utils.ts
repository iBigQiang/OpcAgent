...
export interface LabelMenuSessionStatus {
  id: string
  label: string
}
...
/** Filter flat session states with the same scoring rules as label choices. */
export function filterSessionStatuses<T extends LabelMenuSessionStatus>(states: T[], filter: string): T[] {
  if (!filter) return states

  const segments = filter.toLowerCase().split('/').map(segment => segment.trim()).filter(Boolean)
  if (segments.length !== 1) return []

  const scored: { state: T; score: number }[] = []
  for (const state of states) {
    const score = segmentScore(state.label, segments[0])
    if (score > 0) scored.push({ state, score })
  }
  scored.sort((a, b) => b.score - a.score || a.state.label.localeCompare(b.state.label))
  return scored.map(({ state }) => state)
}
