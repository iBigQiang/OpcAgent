...
/** Browser-safe color resolver kept with the renderer label components.
 * The shared package currently exports types but not the color resolver barrel. */
export function resolveLabelColor(color: EntityColor, isDark: boolean): string {
  if (typeof color !== 'string') return isDark ? (color.dark ?? color.light) : color.light

  const [name, opacity] = color.split('/')
  const base = `var(--${name})`
  if (!opacity) return base
  const percentage = Math.max(0, Math.min(100, Number(opacity)))
  return `color-mix(in srgb, ${base} ${percentage}%, transparent)`
}
...
export function LabelIcon({ label, size = 'sm', hasChildren, className }: LabelIconProps) {
  ...
  const resolvedColor = label.color
    ? resolveLabelColor(label.color, isDark)
    : undefined
