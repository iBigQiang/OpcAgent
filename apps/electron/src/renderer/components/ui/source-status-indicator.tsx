...
const STATUS_CONFIG: Record<SourceConnectionStatus, {
  ...
  connected: {
    ...
    descriptionKey: 'sourceStatus.connectedDescription',
  },
  needs_auth: {
    color: 'bg-info',
    pulseColor: 'bg-info/80',
    labelKey: 'sourceStatus.needsAuthentication',
    descriptionKey: 'sourceStatus.needsAuthenticationDescription',
  },
  failed: {
    color: 'bg-destructive',
    pulseColor: 'bg-destructive/80',
    labelKey: 'sourceStatus.connectionFailed',
    descriptionKey: 'sourceStatus.connectionFailedDescription',
  },
  untested: {
    color: 'bg-foreground/40',
    pulseColor: 'bg-foreground/30',
    labelKey: 'sourceStatus.notTested',
    descriptionKey: 'sourceStatus.notTestedDescription',
  },
  local_disabled: {
    color: 'bg-foreground/30',
    pulseColor: 'bg-foreground/20',
    labelKey: 'sourceStatus.disabled',
    descriptionKey: 'sourceStatus.disabledDescription',
  },
}
...
export function SourceStatusIndicator({
  ...
  const tooltipDescription = status === 'failed' && errorMessage
    ? `${t(config.descriptionKey)}: ${errorMessage}`
