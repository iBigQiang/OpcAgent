...
import { matchesAutomationFilter, parseAutomationsConfig } from '../types'

describe('automation sidebar categories', () => {
  it('matches scheduled, event-triggered, and agentic routes exactly', () => {
    expect(matchesAutomationFilter('SchedulerTick', { kind: 'scheduled' })).toBe(true)
    expect(matchesAutomationFilter('LabelAdd', { kind: 'scheduled' })).toBe(false)
    expect(matchesAutomationFilter('LabelAdd', { kind: 'app' })).toBe(true)
    expect(matchesAutomationFilter('SchedulerTick', { kind: 'app' })).toBe(false)
    expect(matchesAutomationFilter('PreToolUse', { kind: 'agent' })).toBe(true)
    expect(matchesAutomationFilter('FlagChange', { kind: 'agent' })).toBe(false)
  })
})
