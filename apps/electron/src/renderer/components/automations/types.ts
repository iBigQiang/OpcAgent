 ...
 * The renderer runs in a browser context and CANNOT import from @opcagent/shared,
...
export function matchesAutomationFilter(event: string, filter?: AutomationListFilter | null): boolean {
  const kind = filter?.kind ?? 'all'
  if (kind === 'all') return true
  if (kind === 'scheduled') return event === 'SchedulerTick'
  if (kind === 'app') return event !== 'SchedulerTick' && (APP_EVENTS as readonly string[]).includes(event)
  return (AGENT_EVENTS as readonly string[]).includes(event)
}
