...
This guide explains how to customize the visual theme of OPCAgent.

## Overview

OPC Agent uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.opcagent/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.opcagent/theme.json` - Override specific colors (app-level)
...
```
~/.opcagent/workspaces/{id}/config.json
```
...
Create `~/.opcagent/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.opcagent/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.opcagent/themes/{name}.json`
...
1. Create `~/.opcagent/theme.json` for overrides or `~/.opcagent/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.opcagent/theme.json` for overrides, `~/.opcagent/themes/` for presets)
